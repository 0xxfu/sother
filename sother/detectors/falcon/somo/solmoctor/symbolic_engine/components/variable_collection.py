...
from sother.detectors.falcon.somo.solmoctor.core.cfg.flags import EdgeFlag
...
class VariableCollection:
    ...
    def _create_var_by_type(self, var: TNonSSAVar) -> T.Union[Bool, String, Int]:
        ...
        if isinstance(
            var, (TemporaryVariable, SolidityVariable, ReferenceVariable, TupleVariable)
        ):
            var_name = var.name
        else:
    ...
    def _add_execution_var_to_collection(
        self, var: T.Union[BoolRef, ArithRef], src: TemporaryVariableSSA
    ):
    ...
    def condition(self, condition_op: Condition, flag: EdgeFlag):
        ...
        if flag is EdgeFlag.IF_TRUE:
            ...
            raise Exception(
                f"Unknown Condition Result: {str(flag)}, op: {str(condition_op)}"
            )

    def temporary_var(
        self, temporary_var: TemporaryVariableSSA, is_from_solidity_call: bool
    ):
    ...
    def convert(self, convert_op: TypeConversion):
        ...
        if type(src_variable) == type(lvalue):
            ...
            if (
                lvalue_type == "address"
                or "string" in lvalue_type
                or "bytes" in lvalue_type
            ):
