...
from z3 import Solver, unsat, Z3Exception, unknown

from sother.detectors.falcon.somo.solmoctor.core.cfg import ICFGNode
from sother.detectors.falcon.somo.solmoctor.flags import VulnerabilityFlag
from sother.detectors.falcon.somo.solmoctor.symbolic_engine.result import Constraint
    ...
    def check_result(self) -> T.Union[VulnerabilityFlag, Constraint]:
        ...
        if self._solver.check() in (unsat, unknown):
            ...
            for constraint in model:
                try:
                    constraint_var = (
                        self._operation_processor.variable_collection.get_var_by_name(
                            str(constraint)
                        )
                    )
                    ...
                    origin_slither_var = (
                        self._operation_processor.variable_collection.get_key_by_name(
                            str(constraint)
                        )
                    )

                    solved_constraint.add_constraint(
                        origin_slither_var, model[constraint_var]
                    )
