...
from sother.detectors.falcon.somo.solmoctor import VulnerabilityFlag
from sother.detectors.falcon.somo.solmoctor.core.mdg.utils import (
    ConditionOperationChecker,
)
from sother.detectors.falcon.somo.solmoctor.symbolic_engine import Constraint
...
class ConstraintAnalyzer:
    ...
    def check_constraint(
        ...
        entry_vars: T.List,
        sink_var: StateVariable,
        insecure_path: T.List,
    ) -> VulnerabilityFlag:
        ...
        for entry_var in entry_vars:
            try:
                entry_str += (
                    f"{str(entry_var)}: "
                    + str(constraint.constraint_map[entry_var])
                    + ",\n"
                )
            except KeyError:
                continue
