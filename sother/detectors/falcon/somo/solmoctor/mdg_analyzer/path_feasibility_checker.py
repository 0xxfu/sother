...
from slither.core.declarations import Modifier, Contract
from slither.slithir.operations import InternalCall
from typing_extensions import TypeAlias

from sother.detectors.falcon.somo.solmoctor.core import MDG, EntryPoint, EdgeFlag
from sother.detectors.falcon.somo.solmoctor.flags import (
    VulnerabilityFlag,
    ProtectingModifierStatus,
    ModifierStatus,
)
from sother.detectors.falcon.somo.solmoctor.mdg_analyzer import (
    InsecurePathCheckResult,
    ConstraintAnalyzer,
    ContractAnalysisResult,
)
from sother.detectors.falcon.somo.solmoctor.mdg_analyzer.insecure_path_analyzer import (
    InsecurePathAnalyzer,
)
from sother.detectors.falcon.somo.solmoctor.symbolic_engine import (
    SymbolicEngine,
    Constraint,
)
...
class PathFeasibilityChecker:
    def __init__(self, mdg: MDG) -> None:
        ...
        self.modifier_result_map: T.Dict[
            Modifier, T.Optional[T.List[InsecurePathCheckResult]]
        ] = dict()
    ...
    def _decide_modifier_status_from_check_results(self, modifier: Modifier):
        ...
        check_result_status = [_.result for _ in self.modifier_result_map[modifier]]
    ...
    def _record_modifier_check_result(
        self, target_modifier: Modifier, check_result: InsecurePathCheckResult
    ):
    ...
    def _get_modifier_sink_var(self, modifier_sink, mdg: MDG):
        ...
        for edge in edges:
            src, dst = edge
            if (
                graph.edges[src, dst, 0]["edge_flag"]
                == EdgeFlag.SINK_EDGE_FOR_MODIFIER.name
            ):
                return dst.origin
    ...
    def _obtain_source_modifier(self, insecure_path) -> Modifier:
        ...
        modifier_analyzing_result = self.graph.nodes[insecure_path[-1]][
    ...
    def _obtain_all_source_modifier(self, insecure_path_list) -> T.List[Modifier]:
        ...
        modifier_list: T.List[Modifier] = [
            self._obtain_source_modifier(insecure_path)
            for insecure_path, _ in insecure_path_list
        ]
    ...
    def _obtain_all_protected_modifier_from_insecure_path(
        self, insecure_path
    ) -> T.List[Modifier]:
    ...
    def check_one_insecure_path(
        self,
        insecure_path: T.Tuple,
        protecting_modifier_status: T.Optional[ProtectingModifierStatus],
    ) -> InsecurePathCheckResult:
        ...
        if isinstance(execution_result, VulnerabilityFlag):
            return InsecurePathCheckResult(
                VulnerabilityFlag.SECURE, None, insecure_path, entry_vars
            )
            ...
            self.constraint_analyzer.check_constraint(
                execution_result, entry_vars, sink_var, insecure_path
            )

            # do the final model checking and output the result
            path_check_result: VulnerabilityFlag = (
                self.insecure_path_analyzer.check_path_status(insecure_path)
            )

            # return the check result
            return InsecurePathCheckResult(
                path_check_result, execution_result, insecure_path, entry_vars
            )
    ...
    def check_protecting_modifiers(
        self, protecting_modifiers: T.List
    ) -> ProtectingModifierStatus:
        """
        ...
        for protecting_modifier in protecting_modifiers:
            try:
                all_modifier_status.append(
                    self.all_modifier_status_map[protecting_modifier]
                )
            except KeyError:
                pass
    ...
    def _obtain_entry_paths(
        self, insecure_path_list: T.List
    ) -> T.List[T.Tuple[T.Tuple[T.List], T.Optional[ProtectingModifierStatus]]]:
        ...
        for index in range(len(insecure_path_list)):
            ...
            protecting_modifiers: T.List[
                Modifier
            ] = self._obtain_all_protected_modifier_from_insecure_path(insecure_path)
            ...
            protecting_modifier_status: ProtectingModifierStatus = (
                self.check_protecting_modifiers(protecting_modifiers)
            )

            if protecting_modifier_status in [
                ProtectingModifierStatus.SECURE,
                ProtectingModifierStatus.UNDETERMINED,
            ]:
                pass
            elif protecting_modifier_status in [
                ProtectingModifierStatus.CONDITIONAL,
                ProtectingModifierStatus.VULNERABLE,
            ]:
                insecure_path_entry.append(
                    (insecure_path_list[index], protecting_modifier_status)
                )
            else:
    ...
    def check(self, insecure_path_list: T.List) -> ContractAnalysisResult:
        ...
        all_modifier_list = list(
            set(
                self.mdg.contract.modifiers
                + self.mdg.contract.modifiers_declared
                + self.mdg.contract.modifiers_inherited
            )
        )
        ...
        self.all_modifier_status_map: T.Dict[Modifier, ModifierStatus] = {
            _: ModifierStatus.SECURE for _ in all_modifier_list
        }

        # 2. obtain all the modifiers being listed as the attacking targets.
        all_attacking_target_modifier_list = list(
            set(self._obtain_all_source_modifier(insecure_path_list))
        )
        ...
        while insecure_path_list:
            ...
            for insecure_path_entry in insecure_path_entries:
                ...
                insecure_path_result: InsecurePathCheckResult = (
                    self.check_one_insecure_path(
                        insecure_path, protecting_modifier_status
                    )
                )
                attacking_target: Modifier = self._obtain_source_modifier(
                    insecure_path[0]
                )
                self._record_modifier_check_result(
                    attacking_target, insecure_path_result
                )
