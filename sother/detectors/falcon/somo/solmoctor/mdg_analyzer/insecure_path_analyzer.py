...
from sother.detectors.falcon.somo.solmoctor.core import ICFGNode
from sother.detectors.falcon.somo.solmoctor.core.mdg.condition_node_marker import (
    ConditionWrapper,
)
from sother.detectors.falcon.somo.solmoctor.flags import VulnerabilityFlag
...
class ConditionNodeModelChecker:
    ...
    def _check_global_bool_variable(self, vars: T.List[StateVariable]) -> bool:
        ...
        return (
            True if list(filter(lambda var: "bool" in str(var.type), vars)) else False
        )
...
class InsecurePathAnalyzer:
    ...
    def obtain_all_condition_node(
        self, insecure_path: T.List[ICFGNode]
    ) -> T.List[ICFGNode]:
        return list(
            filter(
                lambda node: "CONDITION" in str(node)
                or "require" in str(node)
                or "assert" in str(node),
                insecure_path[:-1],
            )
        )
    ...
    def check_path_status(self, insecure_path: T.List[ICFGNode]) -> VulnerabilityFlag:
        ...
        target_variable_used: T.List[StateVariable] = insecure_path[
            -1
