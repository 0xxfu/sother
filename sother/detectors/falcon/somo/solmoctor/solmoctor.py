...
from sother.detectors.falcon.somo.solmoctor.core import MDGBuilder, InsecurePathSearcher
from sother.detectors.falcon.somo.solmoctor.flags import ContractFlag
from sother.detectors.falcon.somo.solmoctor.mdg_analyzer import (
    ContractAnalyzer,
    PathFeasibilityChecker,
    ContractAnalysisResult,
)
from sother.detectors.falcon.somo.solmoctor.symbolic_engine import SymbolicEngine
...
class SolMoctor:
    ...
    def check(
        self, contract, slither_obj
    ) -> T.Tuple[ContractFlag, T.Optional[ContractAnalysisResult]]:
        ...
        contract_analysis_result: ContractAnalysisResult = (
            multiple_modifier_checker.check(insecure_path_list)
        )
        contract_status: ContractFlag = self.contract_analyzer.check_contract(
