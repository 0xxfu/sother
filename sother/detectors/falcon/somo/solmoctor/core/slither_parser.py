...
from sother.detectors.falcon.somo.solmoctor.exception import (
    SlitherParsingError,
    ContractVersionNotFound,
)
from sother.detectors.falcon.somo.solmoctor.utils import SolcSwitcher, ContractHelper
...
class SlitherParser:
    ...
    def parse(self, contract_path: str, setting_json: str):
        ...
        try:
            slither = Slither(target=contract_path, solc=self.solc)
            return slither.get_contract_from_name(helper.main_contract)[0], slither
        except Exception as e:
            raise SlitherParsingError(
                f"Parsing smart contract into slither obj failed,"
    ...
    def get_version(self, version_str: str) -> str:
        ...
        if res:
            ...
            raise ContractVersionNotFound(
                f"Can not get compiler version: {version_str}."
            )
