...
from sother.detectors.falcon.somo.solmoctor.core.mdg import MDG
...
class InsecurePathSearcher:
    ...
    def search(self, taint_seqs: T.List) -> T.List:
        ...
        for modifier_taint_seqs in list(taint_seqs.values()):
            ...
            for taint_seq, var_flows_to_sink in modifier_taint_seqs:
                ...
                constructor_var: StateVariableWrapper = self._get_constructor_var(
                    taint_seq[-1]
                )
                ...
                for modifier_taint_seq in all_modifier_taint_seqs:
                    contacted_taint_seqs = self._contact_taint_seqs(
                        taint_seq, modifier_taint_seq
                    )
        ...
        for taint_index in range(len(final_taint_list)):
            ...
            for taint_node in taint_seqs:
                ...
                # Some time call a function, try to check whether the function has been restricted by the modifier inclines to update.
                    ...
                    called_function = contract.get_function_from_canonical_name(
                        function_name
                    )
                    if called_function:
                        for modifier in called_function.modifiers:
                            ...
                            if (
                                modifier_sink_scope == modifier.canonical_name
                                or modifier.name in modifier_sink_scope
                            ):
                                secure_flag = True
                                break
    ...
    def _get_constructor_var(
        self, function_sink: FunctionSinkNode
    ) -> StateVariableWrapper:
        edge_map: TEdgeMap = self._node_edge_processor.process_node_edge(
            function_sink, PropagateDirection.BACKWARD, self.graph
        )
        sink_edge_for_function = edge_map[EdgeFlag.SINK_EDGE_FOR_FUNCTION.name][0]
        src_node, dst_node = sink_edge_for_function
        return src_node

    def _get_modifier_taint_seqs(
        self, constructor_node: StateVariableWrapper
    ) -> T.List:
        edge_map: TEdgeMap = self._node_edge_processor.process_node_edge(
            constructor_node, PropagateDirection.BACKWARD, self.graph
        )
        ...
        for sink_edge in sink_edge_for_modifier:
            ...
            modifier_taint_seqs: T.List[T.List] = self.graph.nodes[modifier_sink][
                "modifier_taint_seq_list"
            ]
    ...
    def _process_var_flow_to_sink(self, var_flow_to_sink) -> T.List:
        ...
        for var in var_flow_to_sink:
            ...
            # The constant or enums could not be altered, so filter them out.
                ...
                type(var) == Contract
                or type(var) == EnumContract
                or type(var) == ElementaryType
            ):
                continue
            else:
        ...
        return list(
            filter(lambda var: not isinstance(var, TemporaryVariable), non_ssa_vars)
        )
