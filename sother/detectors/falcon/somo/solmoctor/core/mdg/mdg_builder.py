...
from sother.detectors.falcon.somo.solmoctor.core.cfg.icfg_ssa import ICFG_SSA
...
from sother.detectors.falcon.somo.solmoctor.core.mdg.analyze_result import (
    ModifierAnalyzingResultAnalyzer,
)
from sother.detectors.falcon.somo.solmoctor.core.mdg.analyzer import (
    FunctionTaintAnalyzer,
    ModifierTaintAnalyzer,
)
from sother.detectors.falcon.somo.solmoctor.core.mdg.condition_node_marker import (
    ConditionNodeMarker,
)
from sother.detectors.falcon.somo.solmoctor.core.mdg.utils import (
    ...
    MDGUtilizer,
    SinkEdgeManager,
    UsedVariableProcessingResultFlag,
)
...
class MDGBuilder:
    def __init__(self) -> None:
        ...
        self._modifier_result_analyzer: ModifierAnalyzingResultAnalyzer = (
            ModifierAnalyzingResultAnalyzer()
        )
    ...
    def _build_from_one_modifier(
        ...
        modifier_sinks: T.List[
            ModifierSinkNode
        ] = self._modifier_taint_analyzer.analyze(modifier, icfg)
        ...
        insecure_modifier_sinks: T.List[ModifierSinkNode] = list(
            filter(
                lambda modifier_sink: not self._modifier_result_analyzer.is_sink_secure(
                    modifier_sink, graph
                ),
                modifier_sinks,
            )
        ...
        for insecure_modifier_sink in insecure_modifier_sinks:
            ...
            function_taint_seq_dict = self._function_taint_analyzer.analyze(
                insecure_modifier_sink, icfg
            )
            ...
            for state_var in function_taint_seq_dict.keys():
                ...
                for wrapped_result in function_taint_seq_dict[state_var]:
                    ...
                    ) = self._mdg_utilizer.filter_duplicate_seqs(
                    ...
                    for index in range(len(taint_seq_list)):
                        ...
                        running_stack: T.List = [
                            (taint_seq_list[index], variable_flow_to_sink_list[index])
                        ]
                        ...
                        while running_stack:
                            ...
                            is_attacker_reachable: bool = (
                                self._mdg_utilizer.check_function_visibility(
                                    taint_seq, icfg
                                )
                            )
                            ...
                            used_var_map: T.Dict[
                                TaintVariableType, T.List
                            ] = self._mdg_utilizer.process_variable_flow_to_sink(
                                variable_flow_to_sink, state_var
                            )
                            ...
                            for var in reached_state_variables:
                                if (
                                    var
                                    in used_var_map[TaintVariableType.STATE_VARIABLE]
                                ):
                                    used_var_map[
                                        TaintVariableType.STATE_VARIABLE
                                    ].remove(var)

                            # Categorize the input function variable.
                            process_used_var_result: UsedVariableProcessingResultFlag = self._mdg_utilizer.process_used_var_map(
                                used_var_map
                            )

                            # Attackers' input can not flow to the variable sinks.
                            if (
                                process_used_var_result
                                is UsedVariableProcessingResultFlag.ATTACKER_UNREACHABLE
                            ):
                                continue
                            ...
                            # NOTE: carefully debug here and reduce duplicate insecure paths.
                                ...
                                process_used_var_result
                                is UsedVariableProcessingResultFlag.STATE_VARIABLE_USED
                            ):
                                ...
                                for state_var in used_var_map[
                                    ...
                                    function_sinks_map = self._function_taint_analyzer._process_one_state_var(
                                        state_var, icfg, [scope_str]
                                    )
                                    ...
                                    for sink_function in function_sinks_map.keys():
                                        ...
                                        for function_sink in function_sinks_map[
                                            sink_function
                                        ]:
                                            # Propagate the new function sink in the new sink_function,
                                            (
                                                new_taint_sink_seq,
                                                new_variable_flow_to_sink,
                                            ) = self._function_taint_analyzer._propagate_function_sink(
                                                function_sink, sink_function, icfg
                                            )

                                            # record the new taint node sequences and new variables used.
                                            new_taint_sink_seq_list += (
                                                new_taint_sink_seq
                                            )
                                            new_variable_flow_to_sink_list += (
                                                new_variable_flow_to_sink
                                            )

                                # for every taint sequence in the new function sink.
                                for new_taint_index in range(
                                    len(new_taint_sink_seq_list)
                                ):
                                    # For every new taint node sequences and the variables used in the path/slice.
                                    new_taint_seq = new_taint_sink_seq_list[
                                        new_taint_index
                                    ]
                                    new_taint_var = new_variable_flow_to_sink_list[
                                        new_taint_index
                                    ]

                                    # Give the conditional nodes in the new insecure path values (True or False)
                                    new_taint_seq: T.List[
                                        ICFGNode
                                    ] = conditional_node_marker.process_one_taint_seq_list(
                                        new_taint_seq
                                    )

                                    running_stack.append(
                                        (
                                            new_taint_seq + taint_seq,
                                            variable_flow_to_sink + new_taint_var,
                                        )
                                    )
                            ...
                            # The path is utilizable for attackers, it should be examined later.
                                ...
                                process_used_var_result
                                is UsedVariableProcessingResultFlag.ATTACKER_REACHABLE
                            ):
                                insecure_path.append((taint_seq, variable_flow_to_sink))

        # [([]: taint_seq_list, []: variable_flow_to_sink_list)]
        return insecure_path

    def build(
        self, contract: Contract, slither_obj: Slither = None
    ) -> T.Tuple[MDG, T.Dict]:
