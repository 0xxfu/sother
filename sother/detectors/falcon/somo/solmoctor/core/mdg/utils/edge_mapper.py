...
from sother.detectors.falcon.somo.solmoctor.core.cfg import EdgeFlag, ICFGEdge
...
class EdgeMapper:
    def map_edges(self, edges, graph: nx.MultiDiGraph) -> TEdgeMap:
        ...
        edge_map: T.Dict[str, T.List[ICFGEdge]] = {_.name: list() for _ in EdgeFlag}
        ...
        edge_map[EdgeFlag.GENERAL.name] += (
            edge_map[EdgeFlag.IF_TRUE.name] + edge_map[EdgeFlag.IF_FALSE.name]
        )
