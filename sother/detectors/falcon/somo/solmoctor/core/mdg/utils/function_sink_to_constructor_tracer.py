...
class FunctionSinkToConstructorTracer:
    ...
    def trace_from_function_sink(
        self, function_sink: FunctionSinkNode, graph: nx.MultiDiGraph
    ) -> StateVariableWrapper:
        edge_map: TEdgeMap = self._node_edge_processor.process_node_edge(
            function_sink, PropagateDirection.BACKWARD, graph
        )
        sink_edge_for_function: T.Tuple[
            StateVariableWrapper, FunctionSinkNode
        ] = edge_map[EdgeFlag.SINK_EDGE_FOR_FUNCTION.name][0]
