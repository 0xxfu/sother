...
class MDGUtilizer:
    ...
    def filter_duplicate_seqs(self, taint_seq_list, variable_used_list):
        seqs_str_list: T.List[str] = list(
            map(lambda seq: ",".join(map(str, seq)), taint_seq_list)
        )
    ...
    def process_variable_flow_to_sink(
        self, variable_flow_to_sink_list: T.List, state_var: StateVariable = None
    ):
        used_var_map: T.Dict[TaintVariableType, T.List] = {
            _: list() for _ in TaintVariableType
        }
        ...
        for var in variable_flow_to_sink_list:
            ...
            if isinstance(var, LocalIRVariable):
                if self._parameter_checker.is_parameter(var, var.function):
                    used_var_map[TaintVariableType.FUNCTION_PARAMETER].append(
                        var.non_ssa_version
                    )
                else:
                    continue
            ...
            if isinstance(var, StateIRVariable):
                used_var_map[TaintVariableType.STATE_VARIABLE].append(
                    var.non_ssa_version
                )

            if isinstance(var, SolidityVariableComposed):
                if (
                    var is UserAccessibleSolidityVariable.msg_data
                    or UserAccessibleSolidityVariable.msg_value
                ):
    ...
    def process_used_var_map(
        self, used_var_map: T.Dict[TaintVariableType, T.List]
    ) -> UsedVariableProcessingResultFlag:
        state_variable_used: T.List[StateVariable] = used_var_map[
            TaintVariableType.STATE_VARIABLE
        ]
        function_parameter_used: T.List[LocalVariable] = used_var_map[
            TaintVariableType.FUNCTION_PARAMETER
        ]
        global_variable_used: T.List[SolidityVariableComposed] = used_var_map[
            TaintVariableType.GLOBAL_VARIABLE
        ]
