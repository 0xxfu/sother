...
class ModifierTaintAnalyzer:
    ...
    def _propagate_taint(
        self,
        modifier_sink: ModifierSinkNode,
        modifier_entry: EntryPoint,
        graph: nx.MultiDiGraph,
        modifier: Modifier,
    ) -> None:
        # backwardly iterate the modifier I-CFG, find the variables used by the modifier_sinks
        running_stack: T.List[ICFGNode] = [(modifier_sink, list(), list())]
        ...
        while running_stack:
            ...
            if not isinstance(working_node, FalconNode) and hasattr(
                working_node, "lvalue"
            ):
                # the variable written by the current node
                # Due to the SSA property, only one variable will be written/assigned.
                variable_write_by_working_node: T.Union[
                    TemporaryVariableSSA, LocalIRVariable
                ] = working_node.lvalue
                ...
                if variable_write_by_working_node in variable_used_by_sink:
                    ...
                    for new_taint_variable in working_node.used:
                        ...
                        if isinstance(new_taint_variable, Constant) or isinstance(
                            new_taint_variable, SolidityVariableComposed
                        ):
                            continue
                        ...
                        if isinstance(new_taint_variable, StateIRVariable):
                            ...
                            if self.parameter_checker.is_parameter(
                                new_taint_variable, modifier
                            ):
                                modifier_ir_parameter_flow_to_sink.append(
                                    new_taint_variable
                                )
                            else:
            ...
            edges_map: TEdgeMap = self._edge_processor.process_node_edge(
        ...
        non_ssa_var_mapper: T.Callable[
            [T.Union[LocalIRVariable, StateIRVariable]],
            T.Union[StateVariable, LocalVariable],
        ] = lambda ir_var: ir_var.non_ssa_version
        state_variable_flow_to_sink: T.List[StateVariable] = list(
            map(non_ssa_var_mapper, state_ir_variable_flow_to_sink)
        )
        modifier_parameter_flow_to_sink: T.List[LocalVariable] = list(
            map(non_ssa_var_mapper, modifier_ir_parameter_flow_to_sink)
        )

        # record the analysis result in an object
        analyzing_result: ModifierSinkAnalyzingResult = ModifierSinkAnalyzingResult(
            state_variable_flow_to_sink,
            modifier_parameter_flow_to_sink,
            modifier_sink,
            modifier,
        )
    ...
    def _process_modifier_taint(
        self,
        modifier_taint: ICFGNode,
        modifier_sink: ModifierSinkNode,
        graph: nx.MultiDiGraph,
    ):
    ...
    def _process_modifier_sink(
        self,
        working_node: ICFGNode,
        graph: nx.MultiDiGraph,
        modifier_sinks: T.List[ModifierSinkNode],
    ):
    ...
    def _obtain_modifier_sinks(
        self,
        modifier_entry: EntryPoint,
        modifier_exit: ExitPoint,
        graph: nx.MultiDiGraph,
    ) -> T.List[ModifierSinkNode]:
        ...
        while running_stack:
            ...
            out_edges_map: TEdgeMap = self._edge_processor.process_node_edge(
                working_node, PropagateDirection.FORWARD, graph
            )

            function_call_edge: T.List[FunctionCallEdge] = out_edges_map[
                EdgeFlag.FUNCTION_CALL.name
            ]
            call_return_edge: T.List[CallReturnEdge] = out_edges_map[
    ...
    def _add_edges_from_modifier_sink_to_constructor(
        self, modifier_sink: ModifierSinkNode, icfg: ICFG_SSA
    ):
        graph: nx.MultiDiGraph = icfg.graph

        # retrieve the variables used.
        analyze_result: ModifierSinkAnalyzingResult = graph.nodes[modifier_sink][
            "modifier_sink_result"
        ]
        used_state_variables: T.List[
            StateVariable
        ] = analyze_result.taint_state_variables

        # set the edges to be added
        to_add_edges: T.List[T.Tuple[ICFGEdge, T.Dict[str, str]]] = [
            (
                modifier_sink,
                icfg.get_special_variable(state_var),
                {"edge_flag": EdgeFlag.SINK_EDGE_FOR_MODIFIER.name},
            )
    ...
    def analyze(self, modifier: Modifier, icfg: ICFG_SSA) -> T.List[ICFGNode]:
        ...
        modifier_sinks: T.List[ModifierSinkNode] = self._obtain_modifier_sinks(
            modifier_entry, modifier_exit, graph
        )
        ...
        modifier_sinks: T.List[ModifierSinkNode] = list(
            filter(
                lambda node: node_attr[node] == modifier_canonical_name, modifier_sinks
            )
        )
