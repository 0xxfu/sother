...
class ICFG:
    def __init__(
        self, origin: Contract, build_special_constructor: bool = True
    ) -> None:
    ...
    def _create_special_constructor(self) -> T.List[StateVariableWrapper]:
        return [
            StateVariableWrapper(_)
            for _ in self._origin.constructor.state_variables_written
        ]

    def _init_to_add_node(
        self, node: T.Union[FalconNode, StateVariableWrapper], scope: str
    ...
    def _build_to_add_node_list_from_function_calls(
        self, functions: T.List[T.Union[FunctionContract, Modifier]]
        ...
        for function in functions:
            ...
            for node in nodes:
                to_add_node_list.append(self._init_to_add_node(node, scope))
    ...
    def _add_edge(self, node: FalconNode, graph: nx.MultiDiGraph):
        ...
        external_call_list: T.List[
            T.Union[FunctionContract, Modifier]
        ] = self._identifier.identify(node=node)
    ...
    def _init_icfg(self) -> nx.MultiDiGraph:
        ...
        to_add_node_list: T.List[
            T.Tuple[FalconNode, T.Dict]
    ...
    def all_functions_used(self) -> T.List[T.Union[FunctionContract, Modifier]]:
        ...
        all_functions: T.List[
            T.Union[FunctionContract, Modifier, T.Tuple[Contract, FunctionContract]]
