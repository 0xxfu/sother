...
class CFG:
    def __init__(self, origin: T.Union[Modifier, FunctionContract]):
    ...
    def _init_cfg(self, origin: T.Union[Modifier, FunctionContract]) -> nx.MultiDiGraph:
        ...
        for node in G.nodes:
            if node.son_true is not None or node.son_false:
                G.add_edges_from(
                    [(node, node.son_true, {"cfg_flag": EdgeFlag.IF_TRUE.name})]
                )
                G.add_edges_from(
                    [(node, node.son_false, {"cfg_flag": EdgeFlag.IF_FALSE.name})]
                )
            else:
