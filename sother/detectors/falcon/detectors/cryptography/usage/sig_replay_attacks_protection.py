...
`Token.transfer` does not return a boolean. Bob deploys the token. Alice creates a contract that interacts with it but assumes a correct ERC20 interface implementation. Alice's contract is unable to interact with Bob's contract."""
    ...
    def _detect(self):
        ...
        for contract in self.contracts:
            if (
                contract.name in ["ECDSA"]
                or contract.is_library
                or contract.is_interface
            ):
                continue
                ...
                for node in f.nodes:
                    for ir in node.irs:
                        ...
                        if (
                            ...
                            and ir.function.full_name == ecrecoverFuncName
                        ):
                            ec_functions.append(f)
                            ec_call_functions.append(f)
                            hashNames = self.get_hash_dependence_from_arguments(
                                ir.arguments[0], f
                            )
                            hashRes = hashNames
                            ...
                            for hash in hashNames:
                                ...
                                for dep in dep_list:
                                    if isinstance(dep, StateVariable) or isinstance(
                                        dep, LocalVariable
                                    ):
            ...
            for f in contract.functions:
                for node in f.nodes:
                    for ir in node.irs:
                        ...
                        ) and not isinstance(ir.function, StateVariable):
        ...
        while not isClean:
            ...
            for contract in self.contracts:
                ...
                for f in contract.functions:
                    ...
                    for node in f.nodes:
                        for ir in node.irs:
                            if isinstance(ir, InternalCall) or isinstance(
                                ir, HighLevelCall
                            ):
        ...
        for ec_func in ec_functions:
            ...
            if (det_results != []) and not self.check_state_function(ec_func):
                ...
                if is_prot:
                    protected_func.append(ec_func)
            if not is_cid_addr:
                is_cd = self.detect_func_chainid_addr_protection(
                    ec_func, dep_args, funcMap
                )
                if is_cd:
                    protected_cid_addr_func.append(ec_func)
                ...
                for ir in node.irs:
                    if isinstance(ir, SolidityCall) and ir.lvalue == argument:
                        ...
                        for sub_argument in ir.arguments:
                            vars = self.get_hash_dependence_from_arguments(
                                sub_argument, function
                            )
                            vars = [v for v in vars if v not in dep_vars]
                            dep_vars = dep_vars + vars
                    elif (
                        isinstance(ir, InternalCall) or isinstance(ir, HighLevelCall)
                    ) and ir.lvalue == argument:
                        func = ir.function
                        dep_list = self.get_function_dependence_argument_from_parameter(
                            func, ir.arguments
                        )
                ...
                if isinstance(ir, Return):
                    for v in ir.values:
                        if not isinstance(v, Constant):
                            dv, _ = self.get_dependence_list(
                                depMap, parameters, v, dep_vars
                            )
                    ...
                    if ir.function and ir.function.name == "chainid()":
                        ...
                        func_dep = [
                            arg for arg in ir.arguments if not isinstance(arg, Constant)
                        ]
                        depMap.setdefault(ir.lvalue, func_dep)
                elif isinstance(ir, TypeConversion):
                    var_list = (
                        [] if isinstance(ir.variable, Constant) else [ir.variable]
                    )
                    depMap.setdefault(
                        ir.lvalue,
                        [var for var in var_list if not isinstance(var, Constant)],
                    )
                    ...
                    if function == ir.function:
                        ...
                        if not ir.function:
                            ...
                            if (ir.function) and isinstance(
                                ir.function, FunctionContract
                            ):
                                if not (ir.function in funcs):
                                    callList = self.get_function_dependence_arguments(
                                        ir.function, funcs
                                    )
                                    for i in range(len(ir.arguments)):
                                        if isinstance(ir.arguments[i], StateVariable):
                                            ...
                                            if ir.function.parameters[
                                                i
                                            ] in callList and not isinstance(
                                                ir.function.parameters[i], Constant
                                            ):
        ...
        for node in function.nodes:
            for ir in node.irs:
                if isinstance(ir, Assignment):
                    ...
                    depMap.setdefault(
                        ir.lvalue, [v for v in ir.read if not isinstance(v, Constant)]
                    )
                elif isinstance(ir, Member):
                    depMap.setdefault(
                        ir.lvalue, [v for v in ir.read if not isinstance(v, Constant)]
                        ...
                        if isinstance(ir.function, StateVariable):
                            ...
                            for i in range(len(ir.function.parameters)):
                                for v in callList:
                                    if ir.function.parameters[
                                        i
                                    ] == v and not isinstance(
                                        ir.arguments[i], Constant
                                    ):
                                        calldep.append(ir.arguments[i])
                                    elif isinstance(v, StateVariable):
                                        calldep.append(v)
                                    elif (
                                        isinstance(v, SolidityFunction)
                                        and v.name == "chainid()"
                                    ):
                                        calldep.append(v)
                                    elif (
                                        isinstance(v, SolidityVariable)
                                        and v.name == "this"
                                    ):
                                        calldep.append(v)
                            depMap.setdefault(ir.lvalue, calldep)
    ...
    def get_dependence_from_map(self, depMap, var):
        ...
        while isChange:
            ...
            for v in dList:
                ...
                if len(subDepList) != 0:
                    dList = dList + [
                        dv for dv in subDepList if dv not in (dList + results)
                    ]
                    if len(dList) > l:
                        isChange = True
        ...
        for node in ec_func.nodes:
            for ir in node.irs:
                ...
                if isinstance(ir, SolidityCall):
                    ...
                    if (
                        ir.function.full_name == "require(bool)"
                        or ir.function.full_name == "assert(bool)"
                        or ir.function.full_name == "require(bool,string)"
                        or ir.function.full_name == "assert(bool,string)"
                    ):
                        reDepList = []
                        for arg in ir.arguments:
                            ...
                            for arg in pair.get("local"):
                                ...
                                if arg in reDepList:
                                    for var in dep_state:
                                        if arg in self.get_dependence_from_map(
                                            depMap, var
                                        ):
                                            isGlobal = True
    ...
    def check_protect_args(self, func):
        ...
        for node in func.nodes:
            for ir in node.irs:
                if isinstance(ir, SolidityCall):
                    if (
                        ir.function.full_name == "require(bool,string)"
                        or ir.function.full_name == "require(bool)"
                        or ir.function.full_name == "assert(bool)"
                        or ir.function.full_name == "assert(bool,string)"
                    ):
                        protect_pair = []
                ...
                if func not in protected_list:
                    ...
                    if (
                        len(
                            [
                                ec_f
                                for ec_f in ec_func_list
                                if ec_f not in protected_list
                            ]
                        )
                        == 0
                    ):
    ...
    def get_dependence_parameter_from_call(self, function, arguments):
        ...
        for argument in arguments:
            if argument in parameters or isinstance(argument, StateVariable):
                ...
                results += [
                    var for var in dep_list if var in parameters and var not in results
                ]
        return results

    def get_arguments_from_call_parameters(
        self, function, called_function, called_function_args_list
    ):
        func_args_list = []
    ...
    def check_state_function(self, function):
        for ir in function.falconir_operations:
            ...
            if (
                ...
                and ir.lvalue in function.contract.state_variables
            ):
                return True
            if isinstance(ir, HighLevelCall) or isinstance(ir, InternalCall):
                if not isinstance(
                    ir.function, StateVariable
                ) and self.check_state_function(ir.function):
                    return True
        return False
