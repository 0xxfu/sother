...
class FnTypeVarChecker(AbstractDetector):
    ...
    ARGUMENT = "functiontype-var"  # slither will launch the detector with slither.py --mydetector
            ...
            for fn in contract.functions_declared:
                ...
                for var in fn.variables_written:
                    ...
                    if (
                        var
                        and var.type
                        and isinstance(var.type, UserDefinedType)
                        and isinstance(var.type.type, Structure)
                    ):
                        for mem_var in var.type.type.elems_ordered:
                            if isinstance(mem_var.type, FunctionType):
                                ...
                                break
                ## slither does not support parsing of asm, which may need to be improved
                ...
                #             print(type(ir), ir)
                if (
                    tainted_fn_var is not None
                    and len(self.detect_assembly(func=fn)) > 0
                ):
                    info = [
                        fn.full_name,
                        " has arbitrary jump with function type variable ",
                        tainted_fn_var,
                        "\n",
                    ]
                    info += ["\t- ", fn, "\n"]
