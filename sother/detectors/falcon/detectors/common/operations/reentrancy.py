    ...
    def analyze_node(self, node, detector):
        ...
        for internal_call in node.internal_calls:
            ...
            if isinstance(internal_call, Function):
                ...
                slitherir_operations += internal_call.all_slitherir_operations()
        contains_call = False
        ...
        ) = Reentrancy.record_all_stateVariable_depencent(node)

        for ir in node.irs + slitherir_operations:
            # 如果ir是一个调用
            if hasattr(ir, "destination"):
                # 核心函数，检查重入的destination是否可跳过检查
                if Reentrancy.check_if_ir_or_node_can_skip(
                    ir,
                    node,
                    node_dependent_statevariable_assignment,
                    all_user_controlled_state_vars,
...
class Reentrancy(AbstractDetector):
    ...
    def func_has_reentrancy_lock(self, func):
        ...
        self.FUNC_LOCK_MAP[func] = any(
            [ModifierUtil.is_reentrancy_lock(modifier) for modifier in func.modifiers]
        )
    ...
    def func_can_be_reentrancy(self, check_func: Function, contract: Contract) -> bool:
        ...
        for func in contract.functions:
            if (
                ...
                or func.view
                or func.pure
                or self.func_has_reentrancy_lock(func)
            ):
                continue
            if self.array_has_intersection(
                func.state_variables_read, target_vars
            ) or self.array_has_intersection(func.state_variables_written, target_vars):
    ...
    def record_all_stateVariable_depencent(node):
            ...
            for ir in n.irs:
                if hasattr(ir, "read"):
                    for ir_read in ir.read:
                        if (
                            isinstance(ir_read, LocalVariable)
                            and hasattr(ir_read.expression, "expression_left")
                            and hasattr(ir_read.expression.expression_left, "value")
                            and isinstance(
                                ir_read.expression.expression_left.value, StateVariable
                            )
                        ):
        ...
        for func in node.function.contract.functions:
            if (
                (not func.visibility in ["public", "external"])
                or func.is_constructor
                or "init" in str(func).lower()
                or str(func) == "slitherConstructorVariables"
                or str(func) == "slitherConstructorConstantVariables"
            ):
                continue
        ...
        node_dependent_statevariable_assignment = [
            x
            for x in node_dependent_statevariable_assignment
            if x not in all_user_controlled_state_vars
        ]
        return node_dependent_statevariable_assignment, all_user_controlled_state_vars

    @staticmethod
    def check_if_ir_or_node_can_skip(
        ir,
        node: Node,
        node_dependent_statevariable_assignment,
        all_user_controlled_state_vars,
        ...
        for n in node.function.nodes:
            if any(
                ...
                for item in n.internal_calls
            ) and ".call" not in str(n):
                for n_ir in n.irs:
                    ...
                    if (
                        isinstance(n_ir, Binary)
                        and hasattr(n_ir, "type")
                        and n_ir.type == BinaryType.NOT_EQUAL
                    ):
                        break
                    input_and_require_param.extend(
                        n.variables_read
                    )  # 认为在这个require中，确实是一个完备的检查，则将此node的所有变量加入，并退出循环
                    break
        ...
        if (
            ...
            and ir.arguments
            and "safeTransferETH" in str(node)
            and any(var in node.function.parameters for var in ir.arguments)
        ):
            return False
        ...
        if (
            ...
            and (
                ...
                (
                    isinstance(ir.arguments[0], StateVariable)
                    and ir.arguments[0] not in all_user_controlled_state_vars
                )
                or any(  # 特殊情况：使用基于interface实例化的library call，第一个参数是destination，即像SafeERC20.safeTransfer这样的调用，不进行重入告警
                    is_dependent(ir.arguments[0], var, ir.node.function)
                    for var in node_dependent_statevariable_assignment
                )
                or any(  # 特殊情况：payable(address)
                    is_dependent(ir.arguments[0], var, ir.node.function)
                    and var not in all_user_controlled_state_vars
                    for var in ir.node.function.state_variables_read
                )
            )
        ):
            return True
        ...
        if any(
            is_dependent(ir.destination, var, node.function.contract)
            and var not in all_user_controlled_state_vars
            for var in node.function.contract.state_variables
        ):
            return True

        # 如果调用的合约地址依赖于某个变量且这个变量被require过，则不进行重入报警
        if any(
            is_dependent(ir.destination, var, node.function)
            for var in input_and_require_param
        ):
            return True
    ...
    def _explore(self, node, visited, skip_father=None):
        ...
        (
            node_dependent_statevariable_assignment,
            all_user_controlled_state_vars,
        ) = self.record_all_stateVariable_depencent(node)
        for ir in node.irs:
            # 如果ir是一个调用
            if hasattr(ir, "destination"):
                # 核心函数，检查重入的destination是否可跳过检查
                if self.check_if_ir_or_node_can_skip(
                    ir,
                    node,
                    node_dependent_statevariable_assignment,
                    all_user_controlled_state_vars,
        ...
        if len(node.function.modifiers) > 0 and self.func_has_reentrancy_lock(
            node.function
        ):
    ...
    def detect_reentrancy(self, contract):
        for function in contract.functions_and_modifiers_declared:
            if not (
                "functioncallwithvalue" in function.name
                or "calloptionalreturn" in function.name
                or "set" in function.name
            ):
                if not function.is_constructor:
                    if not function.view:
                        if not (
                            hasattr(function, "modifiers")
                            and len(function.modifiers) > 0
                            and (
                                any(
                                    "only" in mod.name.lower()
                                    or "lock" in mod.name.lower()
                                    or "reentran" in mod.name.lower()
