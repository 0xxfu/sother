...
class Context:
    def __init__(self):
        ...
        self.checked_increase = (
            False  # necessary condition is the absence of a checked increase.
        )
    ...
    def __str__(self):  # debug
        return (
            ...
            + str(self.index_var)
            + " - continue: "
            + str(self.continue_seen)
            + " - checked_increase: "
            + str(self.checked_increase)
        )
...
def check_function(node: Optional[Node], ctx: Context) -> bool:
    ...
    # continue_seen flag
    if (
        node.type == NodeType.CONTINUE
        and ctx.index_var is not None
        and not ctx.unchecked_increase
    ):  # ctx.index_var is not None makes sure we are inside the loop
    ...
    if node.type == NodeType.STARTLOOP:
        if len(node.fathers) > 0:  # should always be true
            ...
            if not len(
                local_written
            ):  # this happens if the index is left implicitly to 0 e.g. for (uint i; i < ...
                local_written = [
                    main_father.variable_declaration
                ]  # in this case we get the declared one
            if len(local_written) == 1:  # should always be true
                ctx.index_var = local_written[
                    0
                ]  # strong assumption, it might be wrong in weird situations
    ...
    for son in node.sons:
        ret = ret or check_function(
            son, ctx
        )  # ctx passed by reference to collect 'continue_seen' and 'checked_increase' flags among all the previous branches
    return ret
...
class ForContinueIncrement(AbstractDetector):
    ...
    def _detect(self) -> List[Output]:
        ...
        for sl in results_raw:
            info = [f"Found vulnerable for loop in function '", sl, "'\n"]
