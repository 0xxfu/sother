...
class OnlyEOACheck(AbstractDetector):
    ...
    WIKI_RECOMMENDATION = (
        "Do not rely on logic msg.sender == tx.origin when protecting contract logic"
    )
            ...
            for n in fun.nodes:  # в первом приближении нода это строчка
                if n.type == NodeType.IF:
                    ...
                    for i in range(len(varListTx)):
                        if str(n).__contains__(
                            f"{varListMsg[i]} == {varListTx[i]}"
                        ) or str(n).__contains__(f"{varListTx[i]} == {varListMsg[i]}"):
                            return "True"
        except:
            return "False"
        return "False"
