...
from slither.core.expressions import TypeConversion, Identifier
...
"""
    ...
    def checkIfHavePriceManipulation(self, contract: Contract):
        ...
        for function in contract.functions:
            ...
            if (
                ...
                and if_has
                and name != info
                and function.name not in str(func).lower()
            ):
    ...
    def _check_defi_action_nesting(self, func):
        if self._check_func_if_have_uniswap(func)[0]:
            return (
                True,
                self._check_func_if_have_uniswap(func)[1],
                "UNISWAP ACTION",
                self._check_func_if_have_uniswap(func)[2],
            )
        for node in func.nodes:
            for call in node.calls_as_expression:
                if call.called and hasattr(call.called, "value"):
                    if (
                        self._check_function_action_type(call.called.value)[0]
                        != "unknown"
                    ):
                        return (
                            True,
    ...
    def _check_func_if_have_uniswap(self, func):
        for node in func.nodes:
            for call in node.calls_as_expression:
                if (
                    call.called
                    and hasattr(call.called, "member_name")
                    and call.called.member_name in self.UNISWAP_FUNCTION
                ):
    ...
    def _check_function_action_type(self, func):
        ...
        (
            depositDiffscore,
            depositTransferScore,
            depositAssignmentScore,
            depositDependencyScore,
        ) = self._check_function_if_staking_or_deposit_or_collateral(func)
        (
            lendingDiffscore,
            lendingTransferScore,
            lendingAssignmentScore,
            lendingDependencyScore,
        ) = self._check_function_if_lending_or_borrow(func)
        (
            withdrawDiffscore,
            withdrawTransferScore,
            withdrawAssignmentScore,
            withdrawDependencyScore,
        ) = self._check_function_if_withdraw_or_unstake(func)
        (
            liquidateDiffscore,
            liquidateTransferScore,
            liquidateAssignmentScore,
            liquidateDependencyScore,
        ) = self._check_function_if_liquidate(func)
        TransferScoreList = [
            depositTransferScore,
            lendingTransferScore,
            withdrawTransferScore,
        ...
        if (
            max(list_diffscore) > 0.6
            and TransferScoreList[list_diffscore.index(max(list_diffscore))] != 0
        ):
            return list_action[list_diffscore.index(max(list_diffscore))], max(
                list_diffscore
            )
        elif (
            max(list_allscore) > 0.4
            and TransferScoreList[list_allscore.index(max(list_allscore))] != 0
        ):
            return list_action[list_allscore.index(max(list_allscore))], max(
                list_allscore
            )
        else:
    ...
    def _check_call_if_transfer_from_user(self, call, func):
        if (
            call.called
            and hasattr(call.called, "member_name")
            and call.called.member_name in ["safeTransferFrom", "transferFrom"]
        ):
            # The first transfer parameter is msg.sender or the external address passed in
            if (
                (
                    isinstance(call.arguments[0], TypeConversion)
                    and hasattr(call.arguments[0].expression, "value")
                    and isinstance(call.arguments[0].expression.value, SolidityVariable)
                    and call.arguments[0].expression.value.name == "msg.sender"
                )
                or (
                    isinstance(call.arguments[0], Identifier)
                    and isinstance(call.arguments[0].value, SolidityVariableComposed)
                    and call.arguments[0].value.name == "msg.sender"
                )
                or (
                    isinstance(call.arguments[0], Identifier)
                    and call.arguments[0].value in func.parameters
                )
            ):
    ...
    def _check_call_if_transfer_to_user(self, call, func):
        if (
            call.called
            and hasattr(call.called, "member_name")
            and call.called.member_name in ["safeTransfer", "transfer"]
        ):
    ...
    def _check_node_if_mapping_assignment_with_sender_or_param(self, node, func):
        if isinstance(node.expression, AssignmentOperation):
            for ir in node.irs:
                if hasattr(ir, "variables") and any(
                    isinstance(e.type, MappingType) for e in ir.variables
                ):
                    if (hasattr(ir.expression.expression_right, "value")) and (
                        (
                            isinstance(
                                ir.expression.expression_right.value,
                                SolidityVariableComposed,
                            )
                            and ir.expression.expression_right.value.name
                            == "msg.sender"
                        )
                        or ir.expression.expression_right.value in func.parameters
                        and isinstance(
                            ir.expression.expression_right.value, LocalVariable
                        )
                        and ir.expression.expression_right.value.type.type == "address"
                    ):
                        if (
                            hasattr(node, "variables_read")
                            and len(node.variables_read) > 0
                        ):
                            return True, set(node.variables_read) - set(ir.variables)

    # 质押或存入的特点：有向内转账，有使用msgsender或者是传入参数的用户数组操作，有数据依赖
    def _check_function_if_staking_or_deposit_or_collateral(
        self, func: FunctionContract
    ):
        depositDiffScore = difflib.SequenceMatcher(
            a="deposit", b=func.name.lower()
        ).ratio()
        stakeDiffScore = difflib.SequenceMatcher(a="stake", b=func.name.lower()).ratio()
        diffscore = (
            depositDiffScore if depositDiffScore > stakeDiffScore else stakeDiffScore
        )
        ...
        if hasattr(func, "nodes"):
            for node in func.nodes:
                ...
                assin_ret = self._check_node_if_mapping_assignment_with_sender_or_param(
                    node, func
                )
    ...
    def _check_function_if_withdraw_or_unstake(self, func: FunctionContract):
        withdrawDiffScore = difflib.SequenceMatcher(
            a="withdraw", b=func.name.lower()
        ).ratio()
        unstakeDiffScore = difflib.SequenceMatcher(
            a="unstake", b=func.name.lower()
        ).ratio()
        diffscore = (
            withdrawDiffScore
            if withdrawDiffScore > unstakeDiffScore
            else unstakeDiffScore
        )
    ...
    def _check_function_if_lending_or_borrow(self, func: FunctionContract):
        lendingDiffScore = difflib.SequenceMatcher(
            a="lending", b=func.name.lower()
        ).ratio()
        borrowDiffScore = difflib.SequenceMatcher(
            a="borrow", b=func.name.lower()
        ).ratio()
        diffscore = (
            lendingDiffScore if lendingDiffScore > borrowDiffScore else borrowDiffScore
        )
    ...
    def _check_function_if_liquidate(self, func: FunctionContract):
        liquidateDiffScore = difflib.SequenceMatcher(
            a="liquidate", b=func.name.lower()
        ).ratio()
        ...
        if hasattr(func, "nodes"):
            for node in func.nodes:
                for call in node.calls_as_expression:
                    ...
                    transfer_from_ret = self._check_call_if_transfer_from_user(
                        call, func
                    )
        ...
        return (
            diffscore,
            transferToScore + transferFromScore,
            assignementToUserScore,
            dependencyScore,
        )
    ...
    def _check_two_arguments_if_dependent(self, callArgs, assignArgs, func):
        ...
        for arg in args:
            if any(
                var
                for vars in assignArgs
                for var in vars
                if is_dependent(arg.value, var, func)
            ):
                return True
