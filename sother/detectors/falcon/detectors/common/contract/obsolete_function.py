...
class ObsoleteUse(AbstractDetector):
    ...
    DEPRECATED_LOW_LEVEL_CALLS = [
        ("callcode", "callcode", "delegatecall", [0.50, 99.0])
    ]
    ...
    def _get_version(self, pragma_str):
        ...
        for i in re_result:
            ...
            version_list = self._fix_less_than(version_list)
            res = [
                float(version_list[0] + "." + version_list[1] + version_list[2]),
    ...
    def detect_deprecation_in_expression(self, expression):
        ...
        for dep_var in self.DEPRECATED_SOLIDITY_VARIABLE:
            if SolidityVariableComposed(
                dep_var[0]
            ) in export_values and self._if_include(dep_var[3], self.solc_pragmas):
                results.append(dep_var)
        for dep_func in self.DEPRECATED_SOLIDITY_FUNCTIONS:
            if SolidityFunction(dep_func[0]) in export_values and self._if_include(
                dep_func[3], self.solc_pragmas
            ):
    ...
    def detect_deprecated_references_in_node(self, node):
        ...
        for dep_node in self.DEPRECATED_NODE_TYPES:
            if node.type == dep_node[0] and self._if_include(
                dep_node[3], self.solc_pragmas
            ):
                ...
                for ir in node.irs:
                    if isinstance(ir, LowLevelCall):
                        for dep_llc in self.DEPRECATED_LOW_LEVEL_CALLS:
                            if ir.function_name == dep_llc[0] and self._if_include(
                                dep_llc[3], self.solc_pragmas
                            ):
    ...
    def _detect(self):
        ...
        for p in pragmas:
            ...
            if len(p.directive) < 1 or p.directive[0] != "solidity":
                continue
            self.solc_pragmas = self._get_correct(
                self._get_version(p.version), self.solc_pragmas
            )
