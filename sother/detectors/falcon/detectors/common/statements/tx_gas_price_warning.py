...
class TxGaspriceWarning(AbstractDetector):
    ...
    ARGUMENT = "tx-gas-price-warning"  # slither will launch the detector with slither.py --detect mydetector
                    ...
                    res.append(
                        self.generate_result(
                            [
                                "In a function ",
                                f,
                                " tx.gasprice (which is set by user) variable is used. Make sure that it can not exploit the contract logic.",
                                "\n",
                            ]
                        )
                    )
