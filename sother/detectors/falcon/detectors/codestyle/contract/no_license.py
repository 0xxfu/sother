...
from sother.detectors.falcon.utils.version import ContractVersion
...
class NoLicense(AbstractDetector):  # pylint: disable=too-few-public-methods
    ...
    ARGUMENT = (
        "no-license"  # slither will launch the detector with slither.py --mydetector
    )
    ...
    def _get_all_pragma_version_map(self):
        ...
        for pragma_directive in self.compilation_unit.pragma_directives:
            ...
            if len(versions) == 1 and len(versions[0]) > 4:
                pragma_version_map[str(pragma_directive.scope)] = ".".join(
                    versions[0][2:]
                )
    ...
    def _detect(self):
        ...
        for contract in self.contracts:
            ...
            if (
                file_scope in pragma_version_map
                and ContractVersion._compareTwoVersion(
                    pragma_version_map.get(file_scope), "0.6.8"
                )
                == 1
            ):
        ...
        for key, source in self.slither.source_code.items():
            if (
                not re.search("SPDX-License-Identifier", source)
                and key in filename_first_contract_map
            ):
                results.append(
                    self.generate_result(
                        [
                            "key",
                            filename_first_contract_map.get(key),
                            " does not specify SPDX license identifier",
                        ]
                    )
                )
