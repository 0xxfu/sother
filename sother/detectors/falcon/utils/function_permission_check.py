        ...
        for ir in node.irs:
            if isinstance(ir, Binary):
                if BinaryType.return_bool(ir.type) and ir.type == BinaryType.NOT_EQUAL:
                    if (
                        ...
                        and ir.variable_left == True
                    ):
                        flag_vars.append(ir.variable_left)
            elif isinstance(ir, Unary):
                if (
                    UnaryType.get_type(str(ir.type).strip(), isprefix=True)
                    == UnaryType.BANG
                ):
            ...
            if len(flag_vars) > 0:
                if isinstance(ir, Assignment):
                    if (
                        ir.lvalue.name in list(map(lambda a: a.name, flag_vars))
                        and isinstance(ir.rvalue, Constant)
                        and ir.rvalue.value == True
                    ):
                        return True
    return False
...
def function_has_caller_check(fn: FunctionContract):
    ...
    if (
        ...
        or fn.pure
        or fn.view
        or (not (fn.visibility in ["public", "external"]))
    ):
        return True
    for node in fn.all_nodes():
        isContractChecks = [
            True
            for ir in node.irs
            if (isinstance(ir, HighLevelCall) or isinstance(ir, LibraryCall))
            and ir.function_name == "isContract"
        ]
        ...
        txOriginChecks = [
            ...
            and (
                (
                    str(ir.variable_left) == "tx.origin"
                    and str(ir.variable_right) == "msg.sender"
                )
                or (
                    str(ir.variable_right) == "tx.origin"
                    and str(ir.variable_left) == "msg.sender"
                )
            )
        ]
